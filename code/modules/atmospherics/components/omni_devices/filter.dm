...
/obj/machinery/atmospherics/omni/filter/error_check()
	if(!input || !output || !filters_new)
		return 1
	if(filters_new.len < 1) //requires at least 1 filter_new ~otherwise why are you using a filter?
		return 1
...
/obj/machinery/atmospherics/omni/filter/process()
	...
	if (power_draw >= 0)
		...
		for(var/datum/omni_port/P in filters_new)
			if(P.network)
				P.network.update = 1
